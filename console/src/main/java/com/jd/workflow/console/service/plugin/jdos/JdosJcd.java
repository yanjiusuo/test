...
@Slf4j
@Service
public class JdosJcd extends JdosAbstract {

    /**
     *
     */
    @Resource
    JdosRestImpl jdosRestImpl;
    ...
    public List<JdosSystemApps> getSystemApps(String erp) {
        ...
        String lsResponse = jdosRest.invokeGet(url,"JDDTEST");
        if (StringUtils.isNotEmpty(lsResponse)) {
            jdosSystemApps.addAll(getJdosSystemApps(lsResponse));
        }
     /*   String kjResponse = jdosRest.invokeGet(url,"JDTTEST");
        if (StringUtils.isNotEmpty(kjResponse)) {
            jdosSystemApps.addAll(getJdosSystemApps(kjResponse));
        }*/
    ...
    private List<JdosSystemApps> getJdosSystemApps(String response) {
        ...
        if (CollectionUtils.isNotEmpty(jdosApps)) {
            ...
            for (String systemName : systemList) {
                ...
                for (JdosApps jdosApp : jdosApps) {
                    if(systemName.equals(jdosApp.getSystemName())){
                        jdosSystem.setTenant(jdosApp.getTenant());
                    }
                }
...
//        String format = "http://api.jcd-gateway.jd.com/api/v2/apps/%s/groups/%s/configs";
    ...
    public String appBuildByDockerfile(String systemName, String appName, String gitUrl, String filePath, String tag, String baseImage, String gitBranch, String erp) {
        ...
        param.put("tag", tag);
        String task = jdosRest.invokePost(url, JSON.toJSONString(param));
        if (StringUtils.isNotEmpty(task)) {
            ...
            if (StringUtils.isNotEmpty(taskId)) {
                ...
                String response = jdosRest.invokePost(callUrl, JSON.toJSONString(callParam));
                return response;
            }
        }
        return null;
