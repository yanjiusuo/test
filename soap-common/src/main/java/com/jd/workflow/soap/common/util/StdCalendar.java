...
public class StdCalendar extends GregorianCalendar {
    ...
    public static StdCalendar fromString(String str) {
        ...
        if (stamp == null) {
            if (!StringHelper.isEmpty(str))
                throw new StdException("util.err_invalid_timestamp").param("ts", str);
            return null;
        }
    ...
    public String toString(String pattern) {
        ...
        String ret = StringHelper.formatDate(this.getTime(), pattern);// FastDateFormat.getInstance(pattern).format(this.getTime());
        return ret;
    }
    ...
    public StdCalendar toMonthOfYear(int month) {
        if (month < 1 || month > 12)
            throw new StdException("util.err_invalid_monthOfYear").param("month", month);
    ...
    public StdCalendar toDayOfWeek(int dayOfWeek) {
        if (dayOfWeek < 0 || dayOfWeek > 7)
            throw new StdException("util.err_invalid_dayOfWeek").param("dayOfWeek", dayOfWeek);
