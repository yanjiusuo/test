...
import com.jd.workflow.soap.common.ResourceUtils;
...
public class SchemaUtils {
    ...
    public static SchemaTypeSystem loadSchemaTypesByWsdlContent(String wsdlContent, SchemaLoader loader){
        try {
            log.debug("Loading schema types from [" + wsdlContent + "]");
            ArrayList<XmlObject> schemas = new ArrayList<XmlObject>(getSchemasByContent(wsdlContent, loader).values());
            return buildSchemaTypes(schemas);
        } catch (Exception e) {
            throw new SoapBuilderException(e);
        }
    ...
    public static Map<String, XmlObject> getSchemasByContent(String wsdlContent, SchemaLoader loader) {
        Map<String, XmlObject> result = new HashMap<String, XmlObject>();
        getSchemasByContent(wsdlContent, result, loader, null /* , false */);
        return result;
    }
    ...
    public static void getSchemasByContent(String content, Map<String, XmlObject> existing, SchemaLoader loader, String tns) {
        String md5Str = Md5Utils.md5(content);
        if (existing.containsKey(md5Str)) {
            return;
        }

        log.debug("Getting schema " + content);
        ...
        try {
            ...
            XmlObject xmlObject = loader.loadXmlObjectByXml(content, options);
            if (xmlObject == null)
                throw new Exception("Failed to load schema from [" + content + "]");
            ...
            if (domNode.getLocalName().equals("schema") && Constants.XSD_NS.equals(domNode.getNamespaceURI())) {
                ...
                if (common && !existing.containsKey(md5Str + "@" + tns))
                    result.put(md5Str + "@" + tns, xmlObject);
                else
                    result.put(md5Str, xmlObject);
            } else {
                existing.put(md5Str, null);
                ...
                for (int i = 0; i < schemas.length; i++) {
                    ...
                    // schemas[i] = XmlObject.Factory.parse( xmlText, options );
                    schemas[i] = XmlUtils.createXmlObject(xmlText, options);
                    schemas[i].documentProperties().setSourceName(md5Str);

                    result.put(md5Str + "@" + (i + 1), schemas[i]);
                }
                ...
                for (int i = 0; i < wsdlImports.length; i++) {
                    ...
                    if (location != null) {
                        if (!location.startsWith("file:") && location.indexOf("://") == -1){
                            //location = joinRelativeUrl(wsdlUrl, location);
                            throw new StdException("wsdl.err_not_support_relative_path").param("path",location);
                        }
