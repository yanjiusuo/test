...
import com.jd.workflow.soap.common.Wsdl11Writer;
import com.jd.workflow.soap.legacy.*;
...
/**
 * 将operation转换为schema描述
 */
public class SoapOperationToJsonTransformer {
    private final static Logger log = LoggerFactory.getLogger(SoapOperationToJsonTransformer.class);
     ...
     */
    public SoapOperationToJsonTransformer(URL wsdlUrl) throws WSDLException {
    ...
    public SoapOperationToJsonTransformer(Definition definition) throws WSDLException {
        WSDLReader reader = new WSDLReaderImpl();
        reader.setFeature("javax.wsdl.verbose", false);
        this.definition = definition;
        this.definitionWrapper = new SchemaDefinitionWrapper(definition);
    }
    /**
    ...
    // ----------------------------------------------------------
    public BuilderJsonType buildSoapMessageFromInput(Binding binding, BindingOperation bindingOperation, SoapContext context) throws Exception {
        SoapVersion soapVersion = getSoapVersion(binding);
        boolean inputSoapEncoded = WsdlUtils.isInputSoapEncoded(bindingOperation);
        SchemaTypeToJsonType xmlGenerator = new SchemaTypeToJsonType(inputSoapEncoded);
       ...
       /* XmlCursor cursor = object.newCursor();
        ...
        BuilderJsonType root = BuilderJsonType.from("object");
        root.setName(soapVersion.getEnvelopeQName());

        if (inputSoapEncoded) {
            root.addXmlAttr("xmlns:xsi",Constants.XSI_NS);
            root.addXmlAttr("xmlns:xsd",Constants.XSD_NS);
           /* cursor.insertNamespace("xsi", Constants.XSI_NS);
            cursor.insertNamespace("xsd", Constants.XSD_NS);*/
        }

        BuilderJsonType body = root.createChild();
        body.setType("object");

        body.setName(soapVersion.getBodyQName());
       /* cursor.toFirstChild();
        cursor.beginElement(soapVersion.getBodyQName());
        cursor.toFirstChild();*/

        if (WsdlUtils.isRpc(definition, bindingOperation)) {
            buildRpcRequest(bindingOperation, soapVersion, body, xmlGenerator);
        } else {
            buildDocumentRequest(bindingOperation, body, xmlGenerator);
        }
        ...
        if (context.isAlwaysBuildHeaders()) {
            ...
            if (bindingInput != null) {
                ...
                addHeaders(soapHeaders, soapVersion, root, xmlGenerator);
            }
        }
        //cursor.dispose();
        return root;
    ...
    // ----------------------------------------------------------
    public BuilderJsonType buildSoapMessageFromOutput(Binding binding, BindingOperation bindingOperation, SoapContext context) throws Exception {
        boolean inputSoapEncoded = WsdlUtils.isInputSoapEncoded(bindingOperation);
        SchemaTypeToJsonType xmlGenerator = new SchemaTypeToJsonType(inputSoapEncoded);
        SoapVersion soapVersion = getSoapVersion(binding);
       ...
       // XmlObject object = XmlObject.Factory.newInstance();
        BuilderJsonType root = new BuilderJsonType();
        root.setType("object");
        root.setName(soapVersion.getEnvelopeQName());
        /*XmlCursor cursor = object.newCursor();
        cursor.toNextToken();
        ...
        if (WsdlUtils.isRpc(definition, bindingOperation)) {
            buildRpcResponse(bindingOperation, soapVersion, body, xmlGenerator);
        } else {
            buildDocumentResponse(bindingOperation, body, xmlGenerator);
        }
    ...
    // --------------------------------------------------------------------------
    private void addHeaders(List<WsdlUtils.SoapHeader> headers, SoapVersion soapVersion, BuilderJsonType parent, SchemaTypeToJsonType xmlGenerator) throws Exception {
        // reposition
       /* cursor.toStartDoc();
        ...
        cursor.toFirstChild();*/
        BuilderJsonType headerType = parent.createChild(soapVersion.getHeaderQName().getLocalPart(),true);
        headerType.setName(soapVersion.getHeaderQName());
        headerType.setType("object");
        ...
        for (int i = 0; i < headers.size(); i++) {
            ...
            if (part != null)
                createElementForPart(part, headerType, xmlGenerator);
            else
                log.error("Missing part for header; " + header.getPart());
        }
    }

    private void buildDocumentResponse(BindingOperation bindingOperation, BuilderJsonType cursor, SchemaTypeToJsonType xmlGenerator)
            throws Exception {
    ...
    private void buildDocumentRequest(BindingOperation bindingOperation, BuilderJsonType cursor, SchemaTypeToJsonType xmlGenerator)
            throws Exception {
    ...
    private void createElementForPart(Part part, BuilderJsonType cursor, SchemaTypeToJsonType xmlGenerator) throws Exception {
        QName elementName = part.getElementName();
        QName typeName = part.getTypeName();

        if (elementName != null) {
            //cursor.beginElement(elementName);
            BuilderJsonType jsonType = cursor.createChild();
            jsonType.setName(elementName);
            ...
            if (definitionWrapper.hasSchemaTypes()) {
                ...
                if (elm != null) {
                    //cursor.toFirstChild();
                    xmlGenerator.createJsonType(elm.getType(), jsonType);
                } else
    ...
    private void buildRpcRequest(BindingOperation bindingOperation, SoapVersion soapVersion, BuilderJsonType parent, SchemaTypeToJsonType xmlGenerator)
            throws Exception {
        ...
        BuilderJsonType cursor = parent.createChild(bindingOperation.getName());
        cursor.setType("object");
        cursor.setName(new QName(ns, bindingOperation.getName()));
        // TODO
        ...
        for (int i = 0; i < inputParts.length; i++) {
            ...
            if (WsdlUtils.isAttachmentInputPart(part, bindingOperation)) {
                ...
                jsonType.addXmlAttr("href", part.getName() + "Attachment");
                ...
                if (definitionWrapper.hasSchemaTypes()) {
                    ...
                    if (typeName != null) {
                        ...
                        if (element != null) {
                            ...
                            xmlGenerator.createJsonType(element.getType(), jsonType);
                        } else
    ...
    private void buildRpcResponse(BindingOperation bindingOperation, SoapVersion soapVersion, BuilderJsonType parent, SchemaTypeToJsonType xmlGenerator)
            throws Exception {
        // rpc requests use the operation name as root element
        BindingOutput bindingOutput = bindingOperation.getBindingOutput();
        ...
        String opName= bindingOperation.getName() + "Response";
        BuilderJsonType cursor = parent.createChild(opName);
        cursor.setType("object");
        cursor.setName(new QName(ns, bindingOperation.getName() + "Response"));
